...
export default function Stream({ contributions, history, className }: StreamProps): React.JSX.Element {
  return (
    <div className={`border border-foreground/20 rounded-lg p-4 ${className ?? ""}`}>
      ...
      <div className="flex flex-col gap-3 overflow-y-auto overflow-x-auto max-h-96">
        {history && history.length > 0 ? (
          history.map((entry, index) => (
            <Typography
              key={index}
              variant="p"
              className="leading-snug text-muted-foreground whitespace-pre-wrap break-words"
            >
              {entry}
            </Typography>
          ))
